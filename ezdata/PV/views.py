...
@login_required
def results(request, id_enseigne):
    global site

    panneau = "OMNIS Power CORTEX 415Wc"
    ...
    result = solution_GT(panneau, conso_perso, profil, territ, surface,
                         installation, puissance, etat_batterie, etat_AutoProd, etat_entre_deux)
    taille = result[0]
    ...
    surface_totale = round(surface_totale, 2)
    auto_conso = calcul_taux_centraleGT(panneau, conso_perso, profil, territ, surface,
                                        installation, puissance, etat_batterie, etat_AutoProd, etat_entre_deux)[1]
    auto_conso = round(auto_conso, 2)
    auto_prod = calcul_taux_centraleGT(panneau, conso_perso, profil, territ, surface,
                                       installation, puissance, etat_batterie, etat_AutoProd, etat_entre_deux)[2]
    auto_prod = round(auto_prod, 2)
    ...
    return render(request, 'dashboard_2.html',
                  {'id_enseigne': id_enseigne, 'site': site, 'nb': range_nb, 'taille': taille, 'nb_modules': nbr_modules, 'surface': surface_totale, 'n': n_solution,
                   'graph1': out, 'graph2': out1, 'graph3': surface_graph, 'EDF': conso_reseau, 'graph4': out2,
                   'graph5': surface_graph1, 'EDF_1': conso_reseau_weekend, 'surface1': surface, 'autoconso': auto_conso,
                   'autoprod': auto_prod})


def data_results(id_enseigne, nsite):
    ...
    return site, range_nb, taille, nbr_modules, surface_totale, n_solution, out, out1, surface_graph, conso_reseau, out2, surface_graph1, conso_reseau_weekend, surface, auto_conso, auto_prod


def data_results_site(id_enseigne, nsite):
    site = nsite
    ...
    return taille, nbr_modules, surface_totale, n_solution, conso_reseau, conso_reseau_weekend, surface, auto_conso, auto_prod
...
def factu(request, id_enseigne):
    ...
    Modules_factu.objects.filter(batiment=rqt2).delete()
    test = calcu_prix(taille, toiture, territ, installation,
                      n_solution, nbr_modules, rqt2)
    test = Modules_factu.objects.filter(batiment=rqt2)

    p1 = total_HA(rqt2)
    p2 = total_Transport(rqt2)
    p3 = total_Marge(rqt2)
    p4 = total_prix(rqt2)
    ...
    return render(request, 'factu.html', {'id_enseigne': id_enseigne, 'site': site, 'nb': range_nb, 'module': test, 'totalHA': p1, 'totalTransport': p2, 'totalMarge': p3, 'totalPrix': p4})


def data_factu(id_enseigne, nsite):
    ...
    return site, range_nb, test, p1, p2, p3, p4


def data_factu_site(id_enseigne, nsite):
    panneau = "OMNIS Power CORTEX 415Wc"
...
def data_mde_site(id_enseigne, nsite):
    site = nsite
    ...
    return Bilan_Economique, Bilan_Energetique, Bilan_Environnemental, invest, emission_avant, emission_apres
...
def data_mobilite_site(id_enseigne, nsite):
    site = nsite
...
# 1er graph : Economies en changant de voiture : passage du thermique a l'eletrique
    ...
    return Bilan_Economique_mobilite, Bilan_Environnemental_mobilite
...
def bilan(request, id_enseigne):
    ...
    Economies_mde = Economies(NbrekWhfacture, Recurrencefacture,
                              Montantfacture, surface, Nbreetages, type, territ)
    Economies_PV = Economies_pv(panneau, conso_perso, profil, territ, surface, installation, puissance, NbrekWhfacture,
                                Recurrencefacture, Montantfacture, Nbreetages, type, etat_batterie, etat_AutoProd, etat_entre_deux)
    Economies_Mobilite = Economies_mobilite(territ, NbreVS, NbkmanVS, NbreVU, NbkmanVU, Presenceparking, Nb_pdc_choisi,
                                            Accessibilite_parking, Optionborne, NbrekWhfacture, Recurrencefacture, Montantfacture, surface, Nbreetages)

    Bilan_avant = Bilan1(panneau, conso_perso, profil, territ, surface, installation, puissance, NbrekWhfacture, Recurrencefacture,
                         Montantfacture, Nbreetages, type, NbreVS, NbkmanVS, NbreVU, NbkmanVU, Nb_batiment, etat_batterie, etat_AutoProd, etat_entre_deux)
...
def data_bilan_site(id_enseigne, nsite):
    site = nsite
    ...
    Total_Energie = round(Energie_mde+Energie_pv, 2)
...
@login_required
def multi_sites_catalogue(request, id_enseigne):
    global conso_perso
...
@login_required
def multi_sites(request, id_enseigne):
    panneau = "OMNIS Power CORTEX 415Wc"
    ...
    for site in range(1, nb+1):
        ...
        coeff_deux_sum += round(Economies_PV[2][2]*Nb_batiment, 2)
        coeff_trois_sum += round(Economies_Mobilite[1][2], 2)
        emission_sans_action_sum += round(Bilan_avant[2]
                                          [8]-(coeff_un_sum+coeff_deux_sum+coeff_trois_sum), 2)
        total = coeff_un_sum+coeff_deux_sum+coeff_trois_sum+emission_sans_action_sum
...
def data_multi_sites(id_enseigne):
    panneau = "OMNIS Power CORTEX 415Wc"
